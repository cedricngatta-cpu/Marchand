...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-slate-950 overflow-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 bg-emerald-600 rounded-xl flex items-center justify-center text-white shadow-lg">
            <Store size={24} />
          </div>
          <span className="font-black text-xl tracking-tighter text-slate-900 dark:text-white uppercase">Inclusion<span className="text-emerald-600">Marchand</span></span>
        </div>
        <button
          onClick={() => router.push('/login')}
          className="bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 px-6 py-2.5 rounded-full font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all"
        >
          Connexion
        </button>
      </nav>
      ...
      <section className="relative pt-40 pb-20 px-6 max-w-7xl mx-auto text-center">
        ...
        >
          <span className="inline-block px-4 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full font-black text-[10px] uppercase tracking-[0.2em] mb-6">
          ...
          <p className="max-w-2xl mx-auto text-lg md:text-xl text-slate-500 dark:text-slate-400 font-medium mb-12">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              ...
              className="w-full sm:w-auto bg-emerald-600 text-white px-10 py-5 rounded-[24px] font-black text-lg uppercase tracking-tight shadow-[0_20px_40px_rgba(5,150,105,0.3)] hover:bg-emerald-700 active:scale-95 transition-all flex items-center justify-center gap-3"
            >
              Créer ma Boutique <ArrowRight size={24} />
            </button>
            <button
              onClick={() => router.push('/login')}
              className="w-full sm:w-auto bg-white dark:bg-slate-800 text-slate-900 dark:text-white px-10 py-5 rounded-[24px] font-black text-lg uppercase tracking-tight border-2 border-slate-100 dark:border-slate-700 active:scale-95 transition-all"
            >
              Déjà membre ?
            </button>
          </div>
      ...
      <section className="py-32 px-6">
        <div className="max-w-5xl mx-auto bg-slate-900 rounded-[50px] p-12 md:p-24 text-center relative overflow-hidden text-white">
          ...
          >
            <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8 leading-tight">Prêt à faire grandir votre commerce ?</h2>
            <p className="text-xl text-slate-400 font-medium mb-12 max-w-xl mx-auto italic">
              Rejoignez des centaines de marchands qui modernisent leur gestion quotidienne avec une simplicité déconcertante.
            </p>
            <button
              onClick={() => router.push('/signup')}
              className="bg-emerald-500 text-white px-12 py-6 rounded-[30px] font-black text-xl uppercase tracking-widest shadow-xl hover:bg-emerald-400 active:scale-95 transition-all"
            >
